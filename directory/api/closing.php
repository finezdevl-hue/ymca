    ...
    if(isset($_POST['action']) && !empty($_POST['action'])) {
        ...
        if($action=="close_account"){
            ...
            try{
                ...
                $sql ="INSERT INTO tbl_member_recieved_old (id, member_id, fees, date, login_id, head, discription,receiveble_id,iswallet,flag,transaction_type,cancel)
                SELECT m.id, m.member_id, m.fees, m.date, m.login_id, m.head, m.discription, m.receiveble_id,m.iswallet,m.flag,m.transaction_type,m.cancel
                FROM tbl_member_recieved AS m LEFT JOIN tbl_member_recievable AS gmm ON m.receiveble_id = gmm.id
                ...
                $sql ="INSERT INTO tbl_member_recievable_old (id, member_id, fees, date, login_id, head, discription, iscomplete, flag, cancel)
                SELECT id, member_id, fees, date, login_id, head, discription,iscomplete,flag,cancel FROM tbl_member_recievable WHERE iscomplete = ?";
                $parameters = [
                    $is_complete,
                ];
                $types="i";
                ...
                $sql ="INSERT INTO tbl_other_recieveble_old (id, date, head, particuler, amount, iscomplete,flag,cancel)
                SELECT id, date, head, particuler, amount, iscomplete,flag,cancel FROM tbl_other_recieveble WHERE iscomplete = ?";
                $parameters = [
                    // $flag,
                    $is_complete,
                ];
                ...
                $sql ="INSERT INTO tbl_other_recieved_old (id,date,head,particuler,amount,recieveble_id,flag,transaction_type,cancel)
                SELECT m.id, m.date, m.head, m.particuler, m.amount, m.recieveble_id,m.flag,m.transaction_type,m.cancel
                ...
                $sql ="INSERT INTO tbl_paid_old (id,date,head,particuler,amount,payable_id,flag,transaction_type,invoice_id,cancel,is_member_trans)
                SELECT m.id, m.date, m.head, m.particuler, m.amount, m.payable_id,m.flag,m.transaction_type,m.invoice_id,m.cancel,m.is_member_trans
                ...
                $sql ="INSERT INTO tbl_payable_old (id, date, head, particuler, amount, iscomplete, flag, invoice_id,cancel,is_member_trans)
                SELECT id, date, head, particuler, amount, iscomplete, flag, invoice_id,cancel,is_member_trans FROM tbl_payable WHERE iscomplete = ?";
                $parameters = [
                    // $flag,
                    $is_complete,
                ];
                ...
                // for deletion from copied table start
                $sql = "DELETE tbl_paid
                FROM tbl_paid
                ...
                // for deletion from copied table end
        ...
        // function resetPayments(){
            try{
                ...
                $sqldatarows=""; 
                    ...
                    $sqldatarows ="SELECT DISTINCT cl.from_year,cl.to_year FROM tbl_member_recieved_old AS mro LEFT JOIN tbl_closing AS cl ON mro.flag=cl.id";
