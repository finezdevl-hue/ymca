        ...
        if($action=="load_data"){
            try{
                ...
                if ($active_yr_res && $row_yr = $active_yr_res->fetch_assoc()) {
                 ...
                 $sqldatarows ="SELECT M.first_name,M.middle_name,M.last_name, MRVD.date, SUM(MRVD.fees) as fees, GROUP_CONCAT(DISTINCT COALESCE(P.name, 'monthly fee') SEPARATOR ', ') as head FROM 
                tbl_member_recieved AS MRVD LEFT JOIN tbl_members AS M ON M.id=MRVD.member_id 
                LEFT JOIN tbl_member_recievable as MRBL ON MRBL.id=MRVD.receiveble_id 
                LEFT JOIN tbl_payment_head_master AS P ON P.id=MRBL.head 
                WHERE MRVD.fees!=0 AND (MRVD.cancel = 0 OR MRVD.cancel IS NULL) AND MRVD.date >= ?
                GROUP BY M.id, MRVD.date
                ORDER BY MRVD.date DESC";
        
                 $sqlcountrows = "SELECT COUNT(*) AS total
                                    FROM (
                                            SELECT M.id FROM 
                                                   tbl_member_recieved AS MRVD LEFT JOIN 
                                                   tbl_members AS M ON M.id = MRVD.member_id LEFT JOIN 
                                                    tbl_member_recievable AS MRBL ON MRBL.id = MRVD.receiveble_id
                                                     LEFT JOIN  tbl_payment_head_master AS P ON P.id = MRBL.head
                                                    WHERE MRVD.fees != 0 AND (MRVD.cancel = 0 OR MRVD.cancel IS NULL) AND MRVD.date >= ?
                                                    GROUP BY M.id, MRVD.date) AS temp;";
