    ...
    // action end
     ...
     if($action=="save_attendance"){ 
            try{
                ...
                WHERE (from_date = ? AND to_date = ? AND member_id = ?)
                OR (from_date = ? AND member_id = ?)";

                $parameters = [
                    $_POST['from_date'],   // first from_date
                    $_POST['to_date'],     // to_date
                    $_POST['member_id'],   // member_id for case 1
                    $_POST['from_date'],   // second from_date
                    $_POST['member_id'],   // member_id for case 2
                ];
                $types = "ssiss";
                ...
                WHERE (date BETWEEN ?  AND ?) AND member_id = ? GROUP BY member_id";
                $parameters = [
                    $_POST['from_date'],
                    $_POST['to_date'],
                     $_POST['member_id'],
