<?php
    ...
    if(isset($_POST['action']) && !empty($_POST['action'])) {
        ...
        if($action=="save_payment"){ 
            try{
                if($_POST['id']==0){
                    $sql ="INSERT INTO tbl_payment_head_master (name, type, pnl, cash,bank) VALUES (?, ?, ?, ?, ?)";
                    ...
                    $sql = "UPDATE tbl_payment_head_master SET  name = ?, type = ?, pnl = ?, cash = ?, bank = ? WHERE id = ?";
        ...
        if($action=="load_data"){
            try{
                ...
                $sqldatarows ="SELECT id,name,type, pnl, cash,bank FROM tbl_payment_head_master";
