    ...
    if(isset($_POST['action']) && !empty($_POST['action'])) {
        ...
        if($action=="load_data"){
            try{
                ...
                $sqldatarows ="SELECT p.id,p.date,p.particuler,p.amount,p.head,p.payable_id,g.type,g.name,cl.from_year,cl.to_year
                FROM tbl_paid_old AS p LEFT JOIN tbl_closing AS cl ON p.flag=cl.id LEFT  JOIN tbl_payment_head_master AS g ON p.head = g.id order by p.date DESC";
        
                $sqlcountrows = "SELECT COUNT(*) AS total FROM tbl_paid_old ";
